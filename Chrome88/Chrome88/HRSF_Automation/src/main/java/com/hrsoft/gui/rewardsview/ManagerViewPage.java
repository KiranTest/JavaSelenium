...
public class ManagerViewPage extends BasePage {
    ...
    private String selectCheckBox        = "(//div[@class='webix_column center left_area webix_last webix_first webix_select_mark']//input[@type='checkbox'])";
    ...
    public ManagerViewPage selectAllCheckBoxes () {
        assertTrue (click (selectCheckBox + "[2]"));
        return this;
    }
    public ManagerViewPage selectMultipleCheckBoxes () {
        assertTrue (click (selectCheckBox + "[1]"));
        assertTrue (click (selectCheckBox + "[2]"));
        assertTrue (click (selectCheckBox + "[3]"));
        return this;
    }
    public ManagerViewPage clickShareAndConfirm () {
        assertTrue (click (share));
        if (isElementVisible("//div[text()='Sharing']"))
    ...
    public boolean isDefaultTabSelected (ManagerViewTabsType tabNameType) {
        Optional <WebElement> defaultTab = getHorizontalTabs ()
                                                               .stream ()
