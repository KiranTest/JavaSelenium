...
public abstract class BasePage {
	...
	@SuppressWarnings("unchecked")
	public <T extends BasePage> T navigateToPage2(Class<T> pageClass, HRSoftPage hrSoftPage) {
		try {
			Constructor<T> constructor = pageClass.getDeclaredConstructor(HRSoftPage.class);
			T page = constructor.newInstance(hrSoftPage);
			page.setHRSoftPage(hrSoftPage);
			@SuppressWarnings("rawtypes")
			ExpectedCondition pageLoadCondition = ((BasePage) page).getPageLoadCondition();
			waitForPageToLoad(pageLoadCondition);
			return page;
	...
	protected HRSoftPage hrSoftPage;

	public void setHRSoftPage(HRSoftPage hrSoftPage) {
	...
	private Duration ofMillis(long millisDuration2) {
	...
	public String clickAndSelectIndex(String select, int index) {
		try {
			WebElement selectElement = waitForElementClickable(select);
			Select dropdown = new Select(selectElement);

			// Check if the index is within the valid range of options
			List<WebElement> options = dropdown.getOptions();
			if (index >= 0 && index < options.size()) {
				dropdown.selectByIndex(index);
				return getText(options.get(index)); // Return the text of the selected option
			} else {
	...
	public void switchToDialogFrame() {
		...
		switchToFrame("//iframe[starts-with(@id,'dialogFrame')]");
	}
	...
	public void navigateToDefaultWindow(WebDriver driver) {
	...
	public static void switchToTabByIndex(WebDriver driver, int tabIndex) {
		...
		if (tabIndex >= 0 && tabIndex < tabs.size()) {
			...
			String newTab = tabs.get(tabIndex);

			// Wait for the new tab to open (adjust timeout as needed)
			WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(30));
			wait.until(ExpectedConditions.numberOfWindowsToBe(tabs.size()));
	...
	public HRSoftPage goToHrSoftPage() {
		switchToDefaultContent();
	...
	/*
	 * private String webixListBox =
	 * "//div[contains(@class,'webix_popup')][contains(@style,'display: block;')]";
	 * private String webixListItem = ".//div[@role='option']"; private String
	 * webixSelectedvalue = ".//div[@class='webix_list_item  webix_selected']";
	 * private String webixListItemByValue = ".//div[@role='option'][text()='%s']";
	 * private String webixListItemByWebixId =
	 * ".//div[@role='option'][@webix_l_id='%s']";
	 */
	...
	public List<String> getColumnValuesOfGrid(int column, String itemid) {
		// grid-28c6a208GRID_6522871416908
		String gridColumnSelector = "//div[@itemid='" + itemid + "']//div[@class='webix_ss_body']//div[@column="
				+ column + "]";
		return findElements(By.xpath(gridColumnSelector + "//div[normalize-space(text())] | " + gridColumnSelector
				+ "//span[normalize-space(text())]")).stream().map(WebElement::getText).collect(Collectors.toList());
	}

	/*
	 * For TCC Dropdowns
	 * findElement("//div[@itemid='MENU_1556762177044176']//child::input").sendKeys(
	 * "MM/dd/yyyy")
	 */
	public void setValueInDropdowns(String itemid, String value) {
		String xpath = "//div[@itemid='" + itemid + "']//child::input";
		findElement(xpath).sendKeys(value);
		findElement(xpath).sendKeys(Keys.ENTER);
	}

	public List<String> getDropdownValues(String itemid) {
		String xpath = "//div[@itemid='" + itemid + "']//div[@class='webix_el_box']//input[@role='combobox']";
		doWait(500);
		assertTrue(click(xpath));
		...
		for (WebElement element : webixPopupBoxForList) {
			if (element.isDisplayed()) {
				returnList.addAll(element.findElements(By.xpath(webixListItem)).stream().map(e -> e.getText())
	...
	public boolean rightClick(String target) {
		waitUntilVisible(target);
		WebElement el = findElement(target);
		try {
	...
	public void clickInlineAction(String grid, int row, String action) {
		assertTrue(click("//div[@itemid='" + grid + "']//div[@aria-rowindex=" + row + "]//a[@title='" + action + "']"));
	}

	public void clickPopupBtn(String text) {
		assertTrue(click("//button[text()='" + text + "']"));
	}

	public boolean waitForElementInvisibleCustom(String target) {
		return waitForElementInvisible(locateBy(target));
	}

	public boolean waitForElementInvisibleCustom(By by) {
		waitForAjaxCalls();
		try {
			return new WebDriverWait(DriverManager.getDriver(), Duration.ofMinutes(5))
					.until(invisibilityOfElementLocated(by));
		} catch (Exception e) {
			// warning (e.toString ());
			info("let's give waitForElementInvisible another try ...");
